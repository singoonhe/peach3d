...
namespace Peach3D
{
    ...
    ITexture* RenderDX::createTexture(const char* name)
    {
        ...
        TextureDX* texture = new TextureDX(mD3DDevice, mDeviceContext, name);
        return texture;
    }

    IProgram* RenderDX::createProgram(uint pId)
    {
        ProgramDX* program = new ProgramDX(mD3DDevice, mDeviceContext, pId);
        return program;
    }

    IObject* RenderDX::createObject(const char* objectName)
    {
        ObjectDX* object = new ObjectDX(mD3DDevice, mDeviceContext, objectName);
        return object;
    }
    ...
    void RenderDX::getWidgetPresetVSSource(const WidgetProgramImpact& impact, std::string* code, std::vector<ProgramUniform>* uniforms)
    {
        bool scrollEnabled = impact.uvIndex < impact.texCount; // enable uv scroll if scrolltexindex is valid
        std::vector<std::string> codeList;
        if (impact.texCount > 0 && scrollEnabled)
        {
            // add object uniform block
            codeList.push_back("cbuffer ObjectConstantBuffer : register(b0)\n"
                "{\n"
                "    float2 pd_scrollUV;\n"
                "};\n");
            uniforms->push_back(ProgramUniform(pdShaderScrollUVUniformName, UniformType::eVector2));
        ...
        codeList.push_back("PixelShaderInput main(VertexShaderInput input)\n"
            ...
            "    vertexShaderOutput.pos = float4(mul(pos, input.modelMat).xy, 0.0f, 1.0f);\n"
    ...
    void RenderDX::getWidgetPresetPSSource(const WidgetProgramImpact& impact, std::string* code, std::vector<ProgramUniform>* uniforms)
    {
        bool scrollEnabled = impact.uvIndex < impact.texCount; // enable uv scroll if scrolltexindex is valid
        std::vector<std::string> codeList;
        // add input struct
        ...
        const char* texNames[] = { pdShaderTexture0UniformName, pdShaderTexture1UniformName,
            pdShaderTexture2UniformName, pdShaderTexture3UniformName };
        for (uint i = 0; i<impact.texCount; ++i)
        {
            char lineCode[100] = {};
            sprintf(lineCode, "Texture2D %s : register(t%d);\nSamplerState pd_sampler%d : register(s%d);\n",
                texNames[i], i, i, i);
        ...
        if (impact.texCount > 0)
        {
            ...
            {
                // use orign uv if current is not scrolled texture
                const char* uvName = impact.uvIndex == i ? "tex_uv" : "input.uv";
                char lineCode[100] = {};
                if (i == 0)
                {
                    sprintf(lineCode, "    float4 final_color = pd_texture%u.Sample(pd_sampler%u, %s);\n", i, i, uvName);
                    codeList.push_back(lineCode);
                }
                else
                {
                    sprintf(lineCode, "    float4 tex_color%u = pd_texture%u.Sample(pd_sampler%u, %s);\n", i, i, i, uvName);
                    codeList.push_back(lineCode);
                    sprintf(lineCode, "    final_color = lerp(final_color, tex_color%u, tex_color%u.a);\n", i, i);
                    codeList.push_back(lineCode);
                }
            }
            codeList.push_back("    final_color = input.color * final_color;\n");
        }
        else
        {
            codeList.push_back("    float4 final_color = input.color;\n");
        }
        if (impact.grayscale > 0)
        {
            codeList.push_back("    float gray = dot(final_color.rgb, float3(0.299, 0.587, 0.114));\n"
                "    final_color = float4(gray, gray, gray, final_color.a);\n");
        }
        codeList.push_back("    return final_color;\n");
        // add main end
        codeList.push_back("}\n");
        std::for_each(codeList.begin(), codeList.end(), [&](const std::string &piece){ *code += piece; });
        Peach3DLog(LogLevel::eInfo, "pixel code: \n%s", code->c_str());
