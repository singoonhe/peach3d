...
namespace Peach3D
{
    ...
    void ProgramGL::updateObjectUnifroms(RenderObjectAttr* attrs, Material* mtl, float lastFrameTime)
    {/*
        if (IPlatform::getSingleton().getRenderFeatureLevel() == RenderFeatureLevel::eGL3)
        {
            if (mProgramUboId != GL_INVALID_INDEX)
            {
                glBindBuffer(GL_UNIFORM_BUFFER, mProgramUboId);
                // map global buffer and copy memory on GL3
                float* mapBuffer = (float*)glMapBufferRange(GL_UNIFORM_BUFFER, 0, mProgramUboSize, GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT);
                if (mapBuffer)
                {
                    for (auto iter=mProgramUniformList.begin(); iter!=mProgramUniformList.end(); ++iter)
                    {
                        ...
                        {
                            // update model matrix
                            memcpy(mapBuffer+mProgramOffsetMap[iter->name]/sizeof(float), attrs->modelMatrix->mat, sizeof(float)*16);
                        }
                        else if (iter->name.compare(pdShaderScrollUVUniformName) == 0 && mtl)
                        {
                            ...
                            memcpy(mapBuffer+mProgramOffsetMap[iter->name]/sizeof(float), &curOffset, sizeof(float)*2);
                        }
                    }
                    glUnmapBuffer(GL_UNIFORM_BUFFER);
                }
        ...
        {
            // set global uniform when render level is GL2, GL3 hase pass global uniform before render
            setUnifromLocationValue(pdShaderProjMatrixUniformName, [&](GLint location)
                                    {
                                        glUniformMatrix4fv(location, 1, GL_FALSE, (*attrs->projMatrix).mat);
                                    });
            setUnifromLocationValue(pdShaderViewMatrixUniformName, [&](GLint location)
                                    {
                                        glUniformMatrix4fv(location, 1, GL_FALSE, (*attrs->viewMatrix).mat);
                                    });
            setUnifromLocationValue(pdShaderAmbientUniformName, [&](GLint location)
                                    {
                                        glUniform4f(location, attrs->ambient->r, attrs->ambient->g, attrs->ambient->b, attrs->ambient->a);
                                    });
            // set object uniform when render level GL2
            for (auto iter=mProgramUniformList.begin(); iter!=mProgramUniformList.end(); ++iter)
            {
                if (iter->name.compare(pdShaderModelMatrixUniformName) == 0)
                {
                    setUnifromLocationValue(iter->name.c_str(), [&](GLint location)
                                            {
                                                glUniformMatrix4fv(location, 1, GL_FALSE, (*attrs->modelMatrix).mat);
                                            });
                }
                else if (iter->name.compare(pdShaderScrollUVUniformName) == 0 && mtl)
                {
                    ...
                    setUnifromLocationValue(iter->name.c_str(), [&](GLint location)
                                            {
                                                glUniform2f(location, curOffset.x, curOffset.y);
                                            });
                }
            }
        }
        ...
        // active textures
        if (mtl) {
            const char* uniformNames[] = {pdShaderTexture0UniformName, pdShaderTexture1UniformName, pdShaderTexture2UniformName, pdShaderTexture3UniformName};
            GLenum texGLs[] = {GL_TEXTURE0, GL_TEXTURE1, GL_TEXTURE2, GL_TEXTURE3};
            mtl->tranverseTextures([&](int index, ITexture* texture){
                GLuint glTextureId = static_cast<TextureGL*>(texture)->getGLTextureId();
                if (glTextureId > 0)
                {
                    // bind one texture
                    glActiveTexture(texGLs[index]);
                    glBindTexture(GL_TEXTURE_2D, glTextureId);
                    setUnifromLocationValue(uniformNames[index], [&index](GLint location)
                                            {
